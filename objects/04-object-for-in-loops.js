...
function printItemAtIndex (x) {
	for (container in pantry) {
		if(pantry[container][x] !== undefined) {
			console.log(`Items at index ${x}:\t${pantry[container][x]}`);
		}
	}
}
...
function printPackageTypeArray(packageType) {
	console.log(`\nItems that are ${packageType}:`);
	for (let item of pantry[packageType]) {
		console.log(`-${item}`);
	}
}

printPackageTypeArray("boxed");
printPackageTypeArray("canned");
